...
const THEME = '#2F5233';
const ACCENT = '#C9A227'; // saffron / gold
...
export default function SatvikMenuPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  if (!menu) {
    return (
      <div className="min-h-screen bg-[#FBF8F1] flex flex-col items-center justify-center px-6 text-center">
        <p className="text-gray-600 mb-4">This menu could not be found.</p>
        <Link href="/amantesatvik" className="text-[#2F5233] font-medium underline">
          Back to Satvik menu
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#FBF8F1]">
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8 py-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#2F5233] focus:ring-opacity-50 bg-white"
            />
            {searchQuery && (
              <button
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-2 scrollbar-hide snap-x snap-mandatory">
            {menu.categories.map(cat => (
              <button
                ...
                style={{
                  backgroundColor: selectedCategory === cat.name ? THEME : undefined
                }}
              >
                {cat.name}
              </button>
          ...
          {filteredItems.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredItems.map((item, index) => {
                const isJain = item.dietary?.includes('jain');
                return (
                <motion.div
                  key={item.id}
                ...
                >
                  ...
                  <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#2F5233]/5 to-transparent rounded-bl-full" />
                  <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-[#C9A227]/5 to-transparent rounded-tr-full" />
                  ...
                  <div className="relative">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          {isJain && (
                            <>
                              ...
                              <span
                                className="text-[11px] sm:text-xs text-white px-2.5 py-1 rounded-full font-medium"
                          ...
                          <button
                            ...
                            className="group/btn flex items-center gap-1.5 text-[#2F5233] hover:text-[#233D26] transition-colors p-1.5 -m-1.5"
                          >
                        ...
                        {item.sizes && item.sizes.length > 0 ? (
                          <div className="flex gap-1.5">
                            {item.sizes.map((size, idx) => (
                              <div
                                key={size.name}
                                className={`inline-flex flex-col items-center px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 rounded-xl shadow-lg shadow-[#2F5233]/20 min-w-[70px] sm:min-w-[80px] md:min-w-[90px] ${
                                  idx === 0
                                    ? 'bg-gradient-to-br from-[#2F5233] to-[#233D26]'
                                    : 'bg-gradient-to-br from-[#3B6B47] to-[#2F5233]'
                                }`}
                              >
                          ...
                          <>
                            {/* Peg sizes row */}
                            <div className="flex gap-1.5">
                              <div className="inline-flex flex-col items-center bg-gradient-to-br from-[#2F5233] to-[#233D26] px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 rounded-xl shadow-lg shadow-[#2F5233]/20 min-w-[70px] sm:min-w-[80px] md:min-w-[90px]">
                              ...
                              <div className="inline-flex flex-col items-center bg-gradient-to-br from-[#3B6B47] to-[#2F5233] px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 rounded-xl shadow-lg shadow-[#2F5233]/20 min-w-[70px] sm:min-w-[80px] md:min-w-[90px]">
                            ...
                            {item.bottlePrice && (
                              <div className="inline-flex flex-col items-center bg-gradient-to-br from-[#233D26] to-[#1A2E1D] px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 rounded-xl shadow-lg shadow-[#2F5233]/20">
                          ...
                          <>
                            ...
                            <div className="inline-flex flex-col items-end bg-gradient-to-br from-[#2F5233] to-[#233D26] px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 md:py-3 rounded-xl shadow-lg shadow-[#2F5233]/20 min-w-[90px] sm:min-w-[110px] md:min-w-[120px]">
                            ...
                            {item.bottlePrice && (
                              <div className="inline-flex flex-col items-end bg-gradient-to-br from-[#233D26] to-[#2F5233] px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 md:py-3 rounded-xl shadow-lg shadow-[#2F5233]/20 min-w-[90px] sm:min-w-[110px] md:min-w-[120px]">
                                <span className="text-[10px] sm:text-xs text-white/80 uppercase tracking-wider font-medium">
                    ...
                    {/* Description */}
                    <div className="relative pl-4 sm:pl-6 md:pl-8 border-l-2 border-[#2F5233]/20">
                      <p className="text-sm sm:text-base text-gray-600 leading-relaxed italic">
                        {item.description?.includes('Make your pizza cheese burst') ? (
                          <>
                            ...
                            <span className="text-[#2F5233] underline decoration-[#2F5233] not-italic font-medium">
      ...
      <div className="text-center py-6 sm:py-8 text-xs sm:text-sm text-gray-500 border-t border-gray-200 mt-6 sm:mt-8">
        ...
        <p className="mb-4" style={{ color: THEME }}>♦ AMANTE SATVIK ♦</p>
        <a
          href="https://restronaut.in"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-[#2F5233] transition-colors"
        >
